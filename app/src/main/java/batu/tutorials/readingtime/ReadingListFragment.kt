...
class ReadingListFragment : Fragment() {
    ...
    private fun getBooksInfo() {
        progressBarReadingList.visibility = View.VISIBLE
        // creating a new array list.
        bookInfoArrayList = ArrayList<BookInfo>()
        val user = FirebaseAuth.getInstance().currentUser
        val database = Firebase.firestore
        val docRef = database.collection("users").document(user!!.uid).collection("reading_list")
        docRef.get().addOnSuccessListener { documents ->
            ...
            if(documents != null) {
                ...
                recyclerViewReadingList.layoutManager = linearLayoutManager
                recyclerViewReadingList.adapter = adapter

                progressBarReadingList.visibility = View.GONE
            } else {
